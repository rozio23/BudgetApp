...
const BalanceBar = () => {
  ...
  const { balance, refreshBalance, refreshVersion } = useBalance();

  // Istniejący useEffect, który odświeża bilans przy zmianie zakresu/zalogowania
  useEffect(() => {
    if (!isAuthenticated) return;
    refreshBalance(RANGE_TO_DAYS[range]);
  }, [isAuthenticated, range, refreshVersion]);

  // --- DODANY KOD: Nasłuchiwanie na sygnał z WebSocketu ---
  useEffect(() => {
    if (!isAuthenticated) return;

    const handleDataRefresh = () => {
      // Wywołujemy odświeżenie z zachowaniem aktualnie wybranego zakresu czasu
      setTimeout(() => {
        refreshBalance(RANGE_TO_DAYS[range]);
      }, 500);
    };
